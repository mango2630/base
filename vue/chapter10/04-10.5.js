  ...
  function patchKeyedChildren(oldVNode, newVNode, container) {
    ...
    while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
      ...
      if (!oldStartVNode) {
        ...
        // 如果新旧节点的头部节点匹配，则说明该旧节点所对应的真实 DOM 节点不需要移动，只需要更新即可。
        patch(oldStartVNode, newStartVNode, container);
        // 更新索引
        oldStartVNode = oldChildren[++oldStartIdx];
        newStartVNode = newChildren[++newStartIdx];
      } else if (oldEndVNode.key === newEndVNode.key) {
        // 如果新旧节点的尾部节点匹配，则说明该旧节点所对应的真实 DOM 节点不需要移动，只需要更新即可。
        ...
        // 移动 DOM: oldStartVNode.el 移动到 oldEndVNode.el 的 DOM 元素之后 
        // 如果旧的一组子节点的头部节点与新的一组子节点的尾部节点匹配，则说明该旧节点所对应的真实 DOM 节点需要移动到尾部。
        ...
        // 为什么需要调用 patch 函数打补丁
        // - 因为 oldEndVNode 和 newStartVNode 是同一个节点，只是位置发生了变化，元素即可
        // - 完善 props & children
        patch(oldEndVNode, newStartVNode, container);
        // 移动 DOM: oldEndVNode.el 移动到 oldStartVNode.el 的 DOM 元素之前
        ...
        // 【为什么一定是 newStart 多余呢】
        // 遍历旧的子节点，尝试寻找与 newStartVNode 的 key 相同的节点
        const idexInOld = oldChildren.findIndex(vnode => vnode.key === newStartVNode.key);
        if (idexInOld > 0) {
          const vnodeToMove = oldChildren[idexInOld];
          patch(vnodeToMove, newStartVNode, container);
          // 移动 DOM: vnodeToMove.el 移动到 oldStartVNode.el 的 DOM 元素之前
          insert(vnodeToMove.el, container, oldStartVNode.el);
          oldChildren[idexInOld] = null;
          // 更新索引
          // newStartVNode = newChildren[++newStartIdx];
        } else { // 如果没有找到，则创建一个新的节点
          // 目前考虑的情况仅仅是新增节点是头部节点
          // if (newStartIdx === 0) {
            patch(null, newStartVNode, container, oldStartVNode.el);
          // } else {
            // patch(null, newStartVNode, container, newStartVNode[newStartIdx - 1].el.nextSibling);
          // }
    ...
    if (newStartIdx <= newEndIdx && oldEndIdx < oldStartIdx) { // 如果新节点的头部索引小于尾部索引，说明新节点有剩余，需要创建
      for(let i = newStartIdx; i <= newEndIdx; i ++) {
        // 什么时候取 el, 什么时候取 el.nextSibling ?
        patch(null, newChildren[i], container, oldEndVNode.el);
      }
    } else if (newStartIdx > newEndIdx && oldStartIdx <= oldEndIdx) {
      // 如果新节点的头部索引大于尾部索引，说明旧节点有剩余，需要卸载
