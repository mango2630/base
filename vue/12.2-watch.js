...
function watch(source, callback, options = {}) {
  ...
  const job = () => {
    newValue = effectFn();
    if (expiredCallback) { // 为什么不是放在 newValue 修改之前呢？？？？？
      expiredCallback();
    }
    callback(newValue, oldValue, onInvalidate);  // 执行 callback, callback 中会重新读取
    oldValue = newValue;
  }
...
watch(
  ...
  (newValue, oldValue) => {
    ...
    immediate: true,
    flush: 'post', // 'pre' dom 更新之前执行 ｜ 'post' dom 更新之后执行 | 'sync' 同步执行
  }
);
obj.age ++;
obj.age ++;
